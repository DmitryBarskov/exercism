...
## [Boutique Suggestions](./boutique-suggestions/README.md)
...
List comprehensions can have filters. Use `for` keyword followed by a generator,
then filter and a do-block.
...
```elixir
for {atom, number} <- [a: 1, b: 2, c: 3, d: 4, e: 5, f: 6],
    rem(number, 2) == 0,
    rem(number, 3) == 0 do
  atom
end
...
See how to define map with pre-defined keys, how to define keyword list.
Note that all keys in keyword list are considered optional.
